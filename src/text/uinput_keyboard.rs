...
use input_linux::{
    UInputHandle, EventKind, SynchronizeKind, Key, KeyState, EventTime,
    InputId, sys
};
...
impl UinputKeyboard {
    pub fn new() -> Result<Self> {
        ...
        match keyboard.create_device() {
            ...
            Err(e) => {
                ...
                warn!("This is likely due to permissions. See documentation for setup instructions.");
    ...
    pub fn type_text(&mut self, text: &str) -> Result<()> {
        ...
        info!("⌨️ Typing text via uinput: '{}' ({} chars)", 
    ...
    fn create_device(&mut self) -> Result<()> {
        ...
        let file = std::fs::OpenOptions::new()
            ...
            .write(true)
            .open("/dev/uinput")
            .map_err(|e| anyhow::anyhow!("Failed to open /dev/uinput: {}. Check permissions.", e))?;
        ...
        let keys_to_enable = [
            ...
            Key::Dot, Key::Comma, Key::Semicolon, Key::Apostrophe, Key::Slash, Key::Backslash,
            Key::Minus, Key::Equal, Key::LeftBrace, Key::RightBrace, Key::Grave,
        ...
        handle.create(&device_id, device_name, 0, &[])
...
pub fn check_uinput_availability() -> Result<()> {
    ...
    match std::fs::OpenOptions::new().write(true).open("/dev/uinput") {
        ...
        Err(e) => {
            Err(anyhow::anyhow!("Cannot access /dev/uinput: {}. Check permissions.", e))
        }
    }
}
